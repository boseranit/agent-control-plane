...
research_program_root: {tmp_path / "programs" / "cli-run"}
""",
        encoding="utf-8",
    )
    return spec_path
...
def test_cli_run_starts_research_run_under_program_root(
    tmp_path: Path,
    ...
    assert exit_code == 0
    run_directory = tmp_path / "programs" / "cli-run" / "runs" / "cli-run"
...
def test_cli_resume_requires_research_program_root(
    capsys: pytest.CaptureFixture[str],
) -> None:
    with pytest.raises(SystemExit) as exc_info:
        main(["resume", "existing-run"])

    assert exc_info.value.code == 2
    assert "--research-program-root" in capsys.readouterr().err


def test_cli_resume_delegates_to_durable_shell_with_research_program_root(
    tmp_path: Path,
    ...
    exit_code = main(
        ["resume", "existing-run", "--research-program-root", str(program_root)]
    )
    ...
    assert seen == [
        ResearchRunInput(
            ...
            research_program_root=str(program_root),
        )
    ]
